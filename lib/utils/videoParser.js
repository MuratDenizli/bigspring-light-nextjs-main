...
export const parseVideoUrls = (videos, platform = null) => {
  ...
  return videos.map(video => {
    ...
    if (currentPlatform === 'android') {
      ...
      if (!parsedUrl.includes('_hls/') && !parsedUrl.endsWith('.m3u8')) {
        const parts = parsedUrl.split('/');
        const fileName = parts[parts.length - 1];
        const pathWithoutFile = parts.slice(0, -1).join('/');
        parsedUrl = `${pathWithoutFile}_hls/${fileName}.m3u8`;
      }
    } else {
 ...
 * @param {string} baseUrl - Base URL (örn: https://api.basincyaralanmasinionle.com/)
 ...
 */
export const processVideoData = (rawVideos, baseUrl = 'https://api.basincyaralanmasinionle.com/', platform = null) => {
  const currentPlatform = platform || detectPlatform();
